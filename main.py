...
def get_csv_filepath():
    try:
        ...
        print(f"Error resolving CSV filepath: {e}")
    return None
...
async def auto_broadcast_job(context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        print(f"Auto-broadcast error caught cleanly: {e}")

# ----------------------------------------------------
# 4. Command & Button Handlers (Bulletproof Error Handling)
# ----------------------------------------------------
...
async def crypto_deposit_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        print(f"Error in crypto_deposit_command: {e}")
...
async def account_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        print(f"Error in account_command: {e}")
...
async def support_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        print(f"Error in support_command: {e}")
...
async def handle_text_and_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if text in ["📊 高概率预测", "📊 High-Prob Predictions", "/top", "/broadcast"]:
        ...
        elif text in ["⏱️ 开启自动播报", "⏱️ Start Auto Broadcast"]:
            ...
            await update.message.reply_text("✅ 已开启每 4 分钟自动高概率播报！")

        elif text in ["🛑 停止播报", "🛑 Stop Broadcast"]:
            if not hasattr(context, 'job_queue') or context.job_queue is None:
            ...
            for job in current_jobs:
                job.schedule_removal()
            await update.message.reply_text("🛑 已成功停止自动播报。")
        ...
        print(f"General message handling error caught cleanly: {e}")
        try:
            await update.message.reply_text("⚠️ 系统处理中，请重新点击菜单按钮。")
        except Exception:
            pass

# ----------------------------------------------------
# 5. Flask Web Server & Anti-Sleep Keep-Alive
# ----------------------------------------------------
app = Flask(__name__)

@app.route('/')
def home():
    return "Mercy TG Bot is running live continuously!", 200
...
async def global_error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    print(f"Unhandled bot exception prevented from crashing loop: {context.error}")

# ----------------------------------------------------
# 7. Main Application Execution
# ----------------------------------------------------
def main():
    # 1. Clear lingering Webhook
    ...
    # 3. Build Telegram Bot Application with auto-reconnect polling
    ...
    print("Starting Telegram bot polling with continuous resilience...")
    
    # Drop pending updates to avoid old command buildup, continuously loop
    application.run_polling(
        drop_pending_updates=True,
        poll_interval=1.0,
        bootstrap_retries=-1  # Keep retrying connection indefinitely if internet drops
    )
